...
if /I not "%PROFILE%"=="light" if /I not "%PROFILE%"=="full" (
    echo Invalid /profile value: %PROFILE%
    exit /b 2
)
...
call :log "Install profile: %PROFILE%"
...
:usage
set "SHOW_HELP=1"
echo Usage: omo_host_bootstrap.cmd [/mode install^|repair^|status] [/profile light^|full] [/distro Ubuntu] [/target C:\AI\omostack] [/port 4096] [/repo URL] [/dry-run]
echo.
...
if /I not "%PROFILE%"=="light" exit /b 0
cscript.exe //nologo //E:JScript "%~f0" distro-exists "%DISTRO%" >nul
if errorlevel 1 exit /b 0
set "INSTALLED_PROFILE="
for /f "usebackq delims=" %%P in (`wsl.exe -d %DISTRO% -u root -- cat /root/.local/state/omo-bootstrap/install-profile 2^>nul`) do set "INSTALLED_PROFILE=%%P"
if /I "%INSTALLED_PROFILE%"=="full" (
    echo ERROR: refusing full -^> light downgrade.
    echo The existing full OmOStack installation was not changed.
    echo Use bootstrap-for-human for repair or reinstall.
    exit /b 1
)
exit /b 0
...
if "%DRY_RUN%"=="1" (
    call :log "[dry-run] wsl.exe -d %DISTRO% -u root -- bash %STAGE_WSL% --mode %MODE% --profile %PROFILE% --target %TARGET_WSL% --repo %REPO% --port %PORT% --yes --host-managed --dry-run"
    cscript.exe //nologo //E:JScript "%~f0" distro-exists "%DISTRO%" >nul
    if errorlevel 1 exit /b 0
    wsl.exe -d %DISTRO% -u root -- bash "%STAGE_WSL%" --mode "%MODE%" --profile "%PROFILE%" --target "%TARGET_WSL%" --repo "%REPO%" --port "%PORT%" --yes --host-managed --dry-run
    if errorlevel 1 exit /b 1
    exit /b 0
)
...
wsl.exe -d %DISTRO% -u root -- bash "%STAGE_WSL%" --mode "%MODE%" --profile "%PROFILE%" --target "%TARGET_WSL%" --repo "%REPO%" --port "%PORT%" --yes --host-managed
exit /b %ERRORLEVEL%
...
call :log "Requested profile: %PROFILE%"
...
wsl.exe -d %DISTRO% -u root -- bash "%STAGE_WSL%" --mode status --profile "%PROFILE%" --target "%TARGET_WSL%" --repo "%REPO%" --port "%PORT%"
exit /b %ERRORLEVEL%
